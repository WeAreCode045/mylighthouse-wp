...
!(function (e, t) {
	"object" == typeof exports && "undefined" != typeof module
		? t(exports, require("@easepick/datetime"))
		: "function" == typeof define && define.amd
		? define(["exports", "@easepick/datetime"], t)
		: t(
				((e =
					"undefined" != typeof globalThis ? globalThis : e || self).easepick =
					e.easepick || {}),
				e.easepick
		  );
})(this, function (e, t) {
	...
	class i {
		...
		getContainerView(e) {
			(this.picker.ui.container.innerHTML = ""),
				this.picker.options.header &&
					this.picker.trigger("render", { date: e.clone(), view: "Header" }),
				this.picker.trigger("render", { date: e.clone(), view: "Main" }),
				this.picker.options.autoApply ||
					this.picker.trigger("render", { date: e.clone(), view: "Footer" });
		}
		getHeaderView(e) {
			const t = document.createElement("header");
			this.picker.options.header instanceof HTMLElement &&
				t.appendChild(this.picker.options.header),
				"string" == typeof this.picker.options.header &&
					(t.innerHTML = this.picker.options.header),
		...
		getFooterView(e) {
			...
			(s.className = "apply-button unit"),
				...
				t.appendChild(i),
				this.picker.ui.container.appendChild(t),
				this.picker.trigger("view", { date: e, target: t, view: "Footer" });
		}
		getCalendarHeaderView(e) {
			...
			(i.className = "month-name"),
				(i.innerHTML = `<span>${e.toLocaleString(this.picker.options.lang, {
					month: "long",
				})}</span> ${e.format("YYYY")}`),
				t.appendChild(i);
			const n = document.createElement("button");
		...
		getCalendarDayView(e) {
			const i = this.picker.options.date
					? new t.DateTime(this.picker.options.date)
					: null,
				n = new t.DateTime(),
			...
			return (
				...
				this.picker.datePicked.length
					? this.picker.datePicked[0].isSame(e, "day") &&
	...
	class n {
			...
			this.picker.options.plugins.forEach((t) => {
				...
				e.sort((e, t) =>
					e.priority > t.priority
						? -1
						: e.priority < t.priority ||
						  e.dependencies.length > t.dependencies.length
						? 1
						: e.dependencies.length < t.dependencies.length
						? -1
						: 0
		...
		removeInstance(e) {
			return (
				e in this.instances && this.instances[e].detach(),
		...
		getPluginFn(e) {
			return [...this.picker.options.plugins]
				.filter((t) => "function" == typeof t && new t().getName() === e)
				.shift();
		}
	}
	class s {
		...
		onClickHeaderButton(e) {
			this.isCalendarHeaderButton(e) &&
				(e.classList.contains("next-button")
		...
		onClickCalendarDay(e) {
			if (this.isCalendarDay(e)) {
				const i = new t.DateTime(e.dataset.time);
		...
		onClick(e) {
			...
			if (t instanceof HTMLElement) {
				...
				this.onClickHeaderButton(e),
					this.onClickCalendarDay(e),
					this.onClickApplyButton(e),
		...
		setDate(e) {
			const i = new t.DateTime(e, this.options.format);
			(this.options.date = i.clone()),
				this.updateValues(),
				this.calendars.length && this.renderAll();
		}
		getDate() {
			return this.options.date instanceof t.DateTime
		...
		updateValues() {
			const e = this.getDate(),
				t =
					e instanceof Date
						? e.format(this.options.format, this.options.lang)
						: "",
				i = this.options.element;
			i instanceof HTMLInputElement
				? (i.value = t)
				: i instanceof HTMLElement && (i.innerText = t);
		}
		hidePicker(e) {
			let t = e.target,
				i = null;
			t.shadowRoot && ((t = e.composedPath()[0]), (i = t.getRootNode().host)),
				this.isShown() &&
					i !== this.ui.wrapper &&
		...
		isCalendarHeaderButton(e) {
			return ["previous-button", "next-button"].some((t) =>
		...
		handleOptions() {
			this.options.element instanceof HTMLElement ||
				...
				this.options.date
					? (this.calendars[0] = new t.DateTime(
							this.options.date,
							this.options.format
					  ))
					: (this.calendars[0] = new t.DateTime());
			...
			return (
				"undefined" != typeof window &&
					(window.innerHeight < s + n.height &&
						s - n.height >= 0 &&
						(s = t.top - i.top - n.height),
					window.innerWidth < a + n.width &&
						t.right - n.width >= 0 &&
						(a = t.right - i.right - n.width)),
				{ left: a, top: s }
			);
		}
	}
	var a = Object.freeze({ __proto__: null, Core: s, create: s });
	(e.Core = s),
		(e.create = s),
		(e.easepick = a),
